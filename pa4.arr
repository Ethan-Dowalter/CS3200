...
fun interpUnexp(rho :: Env, unop :: Unop, e :: Exp) -> Result<Val>:
  ...
  #      | else => err(InterpError("InterpUnexp"))
  #    end
  #  | err(er) => err(InterpError("InterpUnexp"))
  #end
end

# Evaluate a binary expression under a given environment, producing a value.
fun interpBinexp(rho :: Env, binop :: Binop, e1 :: Exp, e2 :: Exp) -> Result<Val>:
  ...
  #cases (Result<Val>) interp(rho, e1):
  #  | ok(a) =>
  #    cases (Result<Val>) interp(rho, e2):
  #      | ok(b) => 
  #        cases (Val) a:
  #          | num(n) => 
  #            cases (Val) b:
  ...
  #          | else => err(InterpError("InterpBinexp"))
  #        end
  #      | err(e) => err(InterpError("InterpBinexp"))
  #    end
  #  | err(e) => err(InterpError("InterpBinexp"))
  #end
end
...
fun interpIfexp(rho :: Env, e1 :: Exp, e2 :: Exp, e3 :: Exp) -> Result<Val>:
  seq(interp(rho, e1), lam(a :: Val):
      cases (Val) a:
        | bool(b) =>
          if b:
  ...
  #      | else => err(InterpError("InterpIfexp"))
  #    end
  #  | err(e) => err(InterpError("InterpIfexp"))
  #end
end

# Evaluate a let expression under a given environment, producing a value.
fun interpLetexp(rho :: Env, x :: String, e1 :: Exp, e2 :: Exp) -> Result<Val>:
  seq(interp(rho, e1), lam(a):
      interp(upd(rho, x, a), e2)
    end)
  #cases (Result<Val>) interp(rho, e1):
  #  | ok(a) => interp(upd(rho, x, a), e2)
  #  | err(e) => err(InterpError("InterpLetexp"))
  #end
end
...
fun interp(rho :: Env, e :: Exp) -> Result<Val>:
  cases (Exp) e:
    ...
    | letx(x, e1, e2) => interpLetexp(rho, x, e1, e2)
  end
end
