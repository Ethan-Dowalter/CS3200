...
# Parse a type
fun parseType(s :: Sexp) -> Result<Type>:
  cases (Sexp) s:
    | s-sym(n) =>
      if eq(n, "bool"):
        ok(tbool)
        ...
        err(ParseError("parseType"))
      end
    | s-list(l) => 
      if eq(l.length(), 3) and eq(l.get(0), S.s-sym("->")):
        lift2(tarrow)(parseType(l.get(1)), parseType(l.get(2)))
      else:
        err(ParseError("parseType"))
      end
    | else => err(ParseError("parseType"))
  end
end
...
fun parseUnop(s :: Sexp) -> Result<Unop>:
  cases (Sexp) s:
    ...
    | else => err(ParseError("parseUnop"))
  end
end

# Parse a Binop.
fun parseBinop(s :: Sexp) -> Result<Binop>:
  cases (Sexp) s:
    | s-sym(n) => 
      if eq(n, "+"):
        ok(add)
...
fun parseIfexp(s :: Sexp) -> Result<Exp>:
  cases (Sexp) s:
    | s-list(l) => 
      if eq(l.length(), 4) and eq(l.get(0), S.s-sym("if")):
        lift3(ite)(parseExp(l.get(1)), parseExp(l.get(2)), parseExp(l.get(3)))
      else:
        err(ParseError("parseIfexp"))
      end
    | else => err(ParseError("parseIfexp"))
  end
end

# Parse a let expression
fun parseLetexp(s :: Sexp) -> Result<Exp>:
  cases (Sexp) s:
    | s-list(l) => 
      if eq(l.length(), 5) and eq(l.get(0), S.s-sym("let")):
        lift4(letx)(parseIdent(l.get(1)), parseType(l.get(2)), parseExp(l.get(3)), parseExp(l.get(4)))
      else:
        err(ParseError("parseLetexp"))
      end
    | else => err(ParseError("parseLetexp"))
  end
end

# Parse an anonymous function expression
fun parseFunexp(s :: Sexp) -> Result<Exp>:
  cases (Sexp) s:
    | s-list(l) =>
      if eq(l.length(), 4) and eq(l.get(0), S.s-sym("fun")):
        lift3(fn)(parseIdent(l.get(1)), parseType(l.get(2)), parseExp(l.get(3)))
      else:
        err(ParseError("parseFun"))
      end
...
fun parseAppexp(s :: Sexp) -> Result<Exp>:
  cases (Sexp) s:
    | s-list(l) =>
      if eq(l.length(), 2):
        lift2(app)(parseExp(l.get(0)), parseExp(l.get(1)))
      else:
        err(ParseError("parseApp"))
      end
    | else => err(ParseError("parseApp"))
  end
end

# Parse a recursive definition expression
fun parseRecexp(s :: Sexp) -> Result<Exp>:
  cases (Sexp) s:
    | s-list(l) =>
      if eq(l.length(), 4) and eq(l.get(0), S.s-sym("rec")):
        lift3(recx)(parseIdent(l.get(1)), parseType(l.get(2)), parseExp(l.get(3)))
      else:
        err(ParseError("parseRec"))
      end
...
fun desugar(e :: Exp) -> ExpC:
  cases (Exp) e:
    ...
    | unexp(op, e1) => 
      cases (Unop) op:
        | unot => unexp(op, desugar(e1))
        | uneg => binexp(sub, val(num(0)), desugar(e1))
      end
    | binexp(op, e1, e2) =>
      cases (Binop) op:
        | conj => ite(desugar(e1), desugar(e2), val(bool(false)))
        | disj => ite(desugar(e1), val(bool(true)), desugar(e2))
        | else => binexp(op, desugar(e1), desugar(e2))
      end
    | ite(e1, e2, e3) => ite(desugar(e1), desugar(e2), desugar(e3))
    | letx(x, t, e1, e2) => app(fn(x, t, desugar(e2)), desugar(e1))
    | fn(x, t, body) => fn(x, t, desugar(body))
    | app(e1, e2) => app(desugar(e1), desugar(e2))
    | recx(x, t, e1) => recx(x, t, desugar(e1))
  end
end
  ...
  cases (Exp) e:
    | val(v) => 
      cases (Val) v:
        | bool(_) => ok(tbool)
        | num(_) => ok(tnum)
        | else => err(TypeError("val"))
      end          
    | ident(s) => lookup(gamma, s)
    | unexp(op, e1) => 
      seq(tycheck(gamma, e1), lam(a):
          if eq(a, tbool):
            ok(tbool)
          else:
            err(TypeError("unexp"))
    ...
    | binexp(op, e1, e2) => 
      if eq(tycheck(gamma, e1), ok(tnum)) and eq(tycheck(gamma, e2), ok(tnum)):
        cases (Binop) op:
          | add => ok(tnum)
          | sub => ok(tnum)
          | mul => ok(tnum)
          ...
          | equ => ok(tbool)
          | lt => ok(tbool)
          | else => err(TypeError("binexp - Not core!!"))
        end
      else:
        err(TypeError("binexp"))
      end
    | ite(e1, e2, e3) => 
      if eq(tycheck(gamma, e1), ok(tbool)):
        seq(tycheck(gamma, e2), lam(a):
            seq(tycheck(gamma, e3), lam(b):
                if eq(a, b):
                  tycheck(gamma, e2) # or equivalently, tycheck(gamma, e3)
                else:
                  err(TypeError("ite"))
                end
              end)
    ...
    | letx(x, t, e1, e2) => err(TypeError("letx - Not core!!"))
    | fn(x, t, body) => 
      seq(tycheck(upd(gamma, x, t), body), lam(a):
          ok(tarrow(t, a))
        end)
    | app(e1, e2) => 
      seq(tycheck(gamma, e1), lam(a :: Type):
          seq(tycheck(gamma, e2), lam(b):
              cases (Type) a:
                | tarrow(t1, t2) => 
                  if eq(t1, b):
                    ok(t2)
                ...
                | else => err(TypeError("app"))
              end
            end)
        end)
    | recx(x, t, e1) =>
      if eq(ok(t), tycheck(upd(gamma, x, t), e1)):
        ok(t)
      else:
        err(TypeError("recx"))
      end
...
# Evaluate a unary expression under a given environment, producing a value.
fun interpUnexp(rho :: Env<Exp>, unop :: Unop, e :: Exp) -> Result<Val>:
  seq(interp(rho, e), lam(a :: Val):
      cases (Val) a:
        | bool(b) => 
          if b:
            ok(bool(false))
        ...
        | else => err(InterpError("InterpUnexp"))
      end
    end)
end

# Evaluate a binary expression under a given environment, producing a value.
fun interpBinexp(rho :: Env<Exp>, binop :: Binop, e1 :: Exp, e2 :: Exp) -> Result<Val>:
  seq(interp(rho, e1), lam(a :: Val):
      seq(interp(rho, e2), lam(b :: Val):
          cases (Val) a:
            | num(n) => 
              cases (Val) b:
                | num(m) => 
                  cases (Binop) binop:
                    ...
                    | div => 
                      if eq(m, 0):
                        err(InterpError("Division by zero!!"))
                      else:
                        ok(num(n / m))
                      end
                    | equ => ok(bool(eq(n, m)))
                    | lt => ok(bool(n < m))
                    | else => err(InterpError("InterpBinexp - Not core!!"))
                  end
                | else => err(InterpError("InterpBinexp"))
              end
            | else => err(InterpError("InterpBinexp"))
          end
        end)
...
# Evaluate an if-then-else expression under a given environment, producing a value.
fun interpIfexp(rho :: Env<Exp>, e1 :: Exp, e2 :: Exp, e3 :: Exp) -> Result<Val>:
  seq(interp(rho, e1), lam(a :: Val):
      cases (Val) a:
        | bool(b) =>
          if b:
        ...
        | else => err(InterpError("InterpIfexp"))
      end
    end)
end

# Evaluate an application expression under a given environment, producing a value.
fun interpApp(rho :: Env<Exp>, e1 :: Exp, e2 :: Exp) -> Result<Val>:
  seq(interp(rho, e1), lam(f :: Val):
      cases (Val) f:
        | clos(myenv, x, body) =>
          seq(interp(rho, e2), lam(a :: Val):
              interp(upd(myenv, x, val(a)), body)
            end)
        | else => err(InterpError("InterpApp"))
      end
    end)
end
...
fun interp(rho :: Env<Exp>, e :: ExpC) -> Result<Val>:
  cases (Exp) e:
    | val(v) => ok(v)
    | ident(s) => seq(lookup(rho, s), lam(a): interp(rho, a) end)
    | unexp(unop, e1) => interpUnexp(rho, unop, e1)
    | binexp(binop, e1, e2) => interpBinexp(rho, binop, e1, e2)
    | ite(e1, e2, e3) => interpIfexp(rho, e1, e2, e3)
    | letx(x, t, e1, e2) => err(InterpError("InterpLetx - Not core!!"))
    | fn(x, t, body) => ok(clos(rho, x, body))
    | app(e1, e2) => interpApp(rho, e1, e2)
    | recx(x, t, e1) => interp(upd(rho, x, e), e1)
  end
end
