...
fun parseLetexp(s :: Sexp) -> Result<Exp>:
  cases (Sexp) s:
    | s-list(l) => 
      if eq(l.length(), 4) and eq(l.get(0), S.s-sym("let")):
        lift3(letx)(parseIdent(l.get(1)), parseExp(l.get(2)), parseExp(l.get(3)))
      else:
...
fun parseFun(s :: Sexp) -> Result<Exp>:
  cases (Sexp) s:
    | s-list(l) =>
      if eq(l.length(), 3) and eq(l.get(0), S.s-sym("fun")):
        lift2(fn)(parseIdent(l.get(1)), parseExp(l.get(2)))
      else:
...
# Convert an s-expression to a Scheme1 Core expression.
...
fun desugar(e :: Exp) -> ExpC:
  cases (Exp) e:
    ...
    | letx(x, e1, e2) => app(fn(x, desugar(e2)), desugar(e1))
...
fun interpUnexp(rho :: Env<Val>, unop :: Unop, e :: Exp) -> Result<Val>:
...
fun interpBinexp(rho :: Env<Val>, binop :: Binop, e1 :: Exp, e2 :: Exp) -> Result<Val>:
...
fun interpIfexp(rho :: Env<Val>, e1 :: Exp, e2 :: Exp, e3 :: Exp) -> Result<Val>:
  seq(interp(rho, e1), lam(a :: Val):
      cases (Val) a:
        | bool(b) =>
          if b:
...
fun interpApp(rho :: Env<Val>, e1 :: Exp, e2 :: Exp) -> Result<Val>:
  seq(interp(rho, e1), lam(f :: Val):
      cases (Val) f:
        | clos(myenv, x, body) =>
          seq(interp(rho, e2), lam(a :: Val):
              interp(upd(myenv, x, a), body)
            end)
...
fun interp(rho :: Env<Val>, e :: ExpC) -> Result<Val>:
  cases (Exp) e:
    ...
    | letx(x, e1, e2) => err(InterpError("InterpLetx - Not core!!"))
    | fn(x, body) => ok(clos(rho, x, body))
