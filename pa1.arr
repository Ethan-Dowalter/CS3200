...
fun my-len<T>(l :: List<T>) -> Nat:
  cases (List) l:
    | empty => O
    | link(f, r) => nat-plus(one, my-len(r))
  end
where:
...
fun my-append<T>(l1 :: List<T>, l2 :: List<T>) -> List<T>:
  cases (List) l1:
    | empty => l2
    | link(f, r) => link(f, my-append(r, l2))
  end
where:
...
fun my-rev<T>(l :: List<T>) -> List<T>:
  ...
  fun my-rev-aux<t>(l2 :: List<t>, len :: Nat) -> List<t>:
    cases (List) l2:
      | empty => empty
      | link(f, r) =>
        cases (Nat) len:
          ...
          | S(nn) => 
            if nn == O:
              link(f, my-rev-aux(r, my-len(r)))
            else:
              my-rev-aux(knil(f, r), nn)
            end
        end
    end
  end
  #Explanation:
  #Once accumulator gets to 1, that means we've called knil for all the elements
  #except the last one, effectively doing a "reverse knil" where the last
  #element gets placed in the first spot instead of the other way around.
  #Then, you have to repeat this process with the now 1 smaller sublist r by
  #recursing until the whole list is reversed.
  
  my-rev-aux(l, my-len(l))
where:
  my-rev([list: 1, 2, 3]) is [list: 3, 2, 1]
  my-rev([list: ]) is [list: ]
  my-rev([list: 1]) is [list: 1]
  my-rev([list: false, true]) is [list: true, false]
  my-rev([list: 1,2,3,4,5,6,7,8,9]) is [list: 9,8,7,6,5,4,3,2,1]
end
...
fun my-flatten<T>(l :: List<List<T>>) -> List<T>:
  cases (List) l:
    | empty => empty
    | link(f, r) => my-append(f, my-flatten(r))
  end
where: 
...
fun isort(l :: List<Number>) -> List<Number>:
  #Auxiliary function with accumulator
  fun isort-aux(l2 :: List<Number>, sortedList :: List<Number>) -> List<Number>:
    cases (List) l2:
      | empty => empty
      | link(f, r) => 
        if r == empty:
          add-elem(f, sortedList)
        else:
          isort-aux(r, add-elem(f, sortedList))
        end
    end
  end

  #Helper function which inserts an element into a sorted list 
  fun add-elem(elem :: Number, l3 :: List<Number>) -> List<Number>:
    cases (List) l3:
      | empty => link(elem, empty)
      | link(f, r) =>
        if elem < f:
          link(elem, l3)
        else:
          link(f, add-elem(elem, r))
        end
    end
    
  where:
    add-elem(1, [list: ]) is [list: 1]
    add-elem(1, [list: 2]) is [list: 1,2]
    add-elem(5, [list: 1,3,7]) is [list:1,3,5,7]
    add-elem(6, [list: 1,4]) is [list: 1,4,6]
  end
