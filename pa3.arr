...
#Convert RBTree to a binary tree without colors
#(I purposely used BSTree as the return type here as opposed to BST
# because I call bst-inv on the return value of this function,
# so I wanted to avoid redundancy)
fun rm-colors-rbt(t :: RBTree<Number>) -> BSTree:
  cases (RBTree) t:
    | rbt-leaf(c) => bst-leaf
    | rbt-node(c, m, l, r) => bst-node(m, rm-colors-rbt(l), rm-colors-rbt(r))
  end
where:
  rm-colors-rbt(r1) is bst-node(2, bst-node(1, bst-leaf, bst-leaf), bst-node(3, bst-leaf, bst-leaf))
end

#Check a RBTree node to make sure it is black
fun rbt-black-check(t :: RBTree<Number>) -> Boolean:
  cases (RBTree) t:
    | rbt-leaf(c) =>
      cases (Color) c:
        | red => false
...
where:
  rbt-leaf(black) satisfies rbt-black-check
  r1 satisfies rbt-black-check
  r2 violates rbt-black-check
end

#Check a RBTree node to make sure it is red and valid (property iii)
fun rbt-red-check(t :: RBTree<Number>) -> Boolean:
  cases (RBTree) t:
    | rbt-leaf(c) => false
    | rbt-node(c, m, l, r) => 
      eq(c, red)
      and rbt-black-check(l)
      and rbt-black-check(r)
  end
where:
  r1 violates rbt-red-check
  r2 satisfies rbt-red-check
end
  
#Checks all paths to leaf nodes and makes sure there are an equal number of black nodes on path
fun check-path(t :: RBTree<Number>) -> Number:
  cases (RBTree) t:
    | rbt-leaf(c) => 1
    | rbt-node(c, m, l, r) =>
      cases (Color) c:
...
#Helper function to call on the whole rbt except for the root
fun rbt-inv2(t :: RBTree<Number>) -> Boolean:
  cases (RBTree) t:
    | rbt-leaf(c) => rbt-black-check(t)
    | rbt-node(c, m, l, r) =>
      rbt-inv2(l)
      and rbt-inv(r)
  end
...
fun rbt-inv(t :: RBTree<Number>) -> Boolean:
  if rbt-black-check(t) and bst-inv(rm-colors-rbt(t)) and not(eq(check-path(t), -1)):
    cases (RBTree) t:
      | rbt-leaf(c) => rbt-black-check(t)
      | rbt-node(c, m, l, r) =>
        rbt-inv2(l)
...
fun stream-sums(s :: Stream<Number>) -> Stream<Number>:
  fun stream-sums-aux(s2 :: Stream<Number>, acc :: Number) -> Stream<Number>:
    lz-link(lz-first(s2) + acc, lam(): stream-sums-aux(lz-rest(s2), lz-first(s2) + acc) end)
  end
  stream-sums-aux(s, 0)
where:
...
fun abs-val(x :: Number, y :: Number) -> Number:
  if x < y:
    y - x
  else:
    x - y
...
fun approximate(tolerance :: Number, s :: Stream<Number>) -> Number:
  if abs-val(lz-first(s), lz-first(lz-rest(s))) < tolerance:
    lz-first(lz-rest(s))
  else:
    approximate(tolerance, lz-rest(s))
  end
where:
...
fun div-results(x :: Result<Number>, y :: Result<Number>) -> Result<Number>:
  cases (Result) x:
    | err(e) => err(e)
    | ok(n) => 
      cases (Result) y:
...
fun eval(env :: Env, e :: Exp) -> Result<Number>:
  cases (Exp) e:
    | Var(x) => lookup(env, x)
    | Num(n) => ok(n)
    | Plus(l, r) => lift2(lam(m :: Number, n :: Number): m + n end)(eval(env, l), eval(env, r))
      #add-results(eval(env, l), eval(env, r))
    | Div(l, r) => div-results(eval(env, l), eval(env, r))
  end
where:
