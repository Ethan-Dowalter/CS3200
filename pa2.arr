...
fun my-maptwice<T>(f :: (T -> T), l :: List<T>) -> List<T>:
  cases (List) l:
    | empty => empty
    | link(x, r) => link(f(f(x)), my-maptwice(f,r))
  end
where:
...
fun num-nodes<A>(b :: BTree<A>) -> Number:
  cases (BTree) b:
    | leaf(x) => 1
    | node(l, r) => 1 + num-nodes(l) + num-nodes(r)
  end
where:
...
fun veval(env :: Env, e :: VExp) -> Number:
  cases (VExp) e:
    | VVar(x) => lookup(env, x)
    | VNum(n) => n
    | VPlus(l, r) => veval(env, l) + veval(env, r)
    | VDiv(l, r) => 
      if veval(env, r) == 0:
        raise("division by zero")
      else:
        veval(env, l) / veval(env, r)
      end
  end
where:
...
where:
  opt(VNum(0)) is VNum(0)
  opt(VNum(5)) is VNum(5)
  opt(VVar("x")) is VVar("x")
  opt(VPlus(VNum(0), VNum(5))) is VNum(5)
  opt(VPlus(VNum(5), VNum(0))) is VNum(5)
  opt(VPlus(VNum(0), VVar("x"))) is VVar("x")
  opt(VPlus(VVar("x"), VNum(0))) is VVar("x")
  opt(VDiv(VNum(0), VNum(5))) is VNum(0)
  opt(VDiv(VNum(0), VVar("x"))) is VNum(0)
  opt(VDiv(VNum(5), VNum(0))) raises "division by zero"
  opt(VDiv(VVar("x"), VNum(0))) raises "division by zero"
  opt(VDiv(VNum(5), VNum(1))) is VNum(5)
  opt(VDiv(VVar("x"), VNum(1))) is VVar("x")
  opt(VDiv(VNum(5), VNum(5))) is VNum(1)
  opt(VDiv(VVar("x"), VVar("x"))) is VNum(1)
  opt(VDiv(VVar("x"), VNum(5))) is VDiv(VVar("x"), VNum(5))
end